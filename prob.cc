...
// randcomb() produces a random combination (i.e., unordered subset) of
...
//
// Though randcomb() is required to fulfill the first-order inclusion
...
//
// randcomb() only calls for one random number generation (this is important
...
std::vector<int>
randcomb(unsigned k, const std::vector<float>& p) {
